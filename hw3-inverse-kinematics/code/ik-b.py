...
def objective(x):
    ...
    :returns: a scalar value with type np.float64 representing the objective
        cost for x
    """
    forward = fk(x, link_lengths)
    return np.square(np.linalg.norm(forward - p_d))
...
def plot_solution(x):
    ...
    #ax.set(xlim=(0, 1), ylim=(0, 1.5), zlim=(-1, 1))

    # show result
    #plt.savefig('../ik-b_solution', bbox_inches='tight')
    plt.show()
