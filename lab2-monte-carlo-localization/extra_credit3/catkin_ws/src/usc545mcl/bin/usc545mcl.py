...
class Particle(object):
    ...
    def UpdateOdom(self, odom_msg):
        ...
        vel_x = vel_x #+ np.random.normal(0, np.sqrt(LINEAR_MODEL_VAR_X))
        vel_y = vel_y #+ np.random.normal(0, np.sqrt(LINEAR_MODEL_VAR_Y))

        [vel_x_m, vel_y_m] = RotateBy(np.array([vel_x, vel_y]), angle)

        pos_x = vel_x_m * dt + self.map_T_particle.translation[0] + np.random.normal(0, np.sqrt(LINEAR_MODEL_VAR_X))
        pos_y = vel_y_m * dt + self.map_T_particle.translation[1] + np.random.normal(0, np.sqrt(LINEAR_MODEL_VAR_Y))

        self.map_T_particle.translation[0] = pos_x
        self.map_T_particle.translation[1] = pos_y

        theta = odom_msg.twist.twist.angular.z
        theta = theta * dt + np.random.normal(0, np.sqrt(ANGULAR_MODEL_VAR))

        theta_m = angle + theta
        self.map_T_particle.rotation = theta_m
    ...
    def UpdateScan(self, scan):
        ...
        #const = np.sqrt(2 * np.pi * SENSOR_MODEL_VAR, dtype=dtype)
        #diff = scan.ranges - sim_ranges
        #likelihood = np.exp(-(diff**2) / (2 * SENSOR_MODEL_VAR), dtype=dtype)
        #likelihood /= const

        #self.weight = np.prod(likelihood, dtype=dtype)
        
        exp = []
        vav_1 = []
        val_2 = []
        for p in range(len(scan.ranges)):
            vav_1.append(-1.0 * (scan.ranges[p] - sim_ranges[p]) ** 2)
            vav_1[p] = vav_1[p] / (2.0 * (SENSOR_MODEL_VAR))
            exp.append(np.exp(vav_1[p], dtype=dtype))
            val_2.append(const * exp[p])

        result = 1

        for p in range(len(val_2)):
            result = result * val_2[p]
...
class ParticleFilter(object):
    ...
    def GetMeanPose(self):
        ...
        x_translations = np.zeros(len(self.particles))
        y_translations = np.zeros(len(self.particles))
        rotations = np.zeros(len(self.particles))
        for i, particle in enumerate(self.particles):
            x_translations[i] = particle.map_T_particle.translation[0]
            y_translations[i] = particle.map_T_particle.translation[1]
            rotations[i] = particle.map_T_particle.rotation

        x_translation_preds = kmeans.fit_predict(x_translations)
        y_translation_preds = kmeans.fit_predict(y_translations)
        rotation_preds = kmeans.fit_predict(rotations)

        x_mode = mode(x_translation_preds)[0][0]
        y_mode = mode(y_translation_preds)[0][0]
        rotation_mode = mode(rotation_preds)[0][0]

        x_out = []
        y_out = []
        rotation_out = []
        for i, particle in enumerate(self.particles):
            if x_translation_preds[i] == x_mode:
                x_out.append((x_translations[i]))
            if y_translation_preds[i] == y_mode:
                y_out.append((y_translations[i]))
            if rotation_preds[i] == rotation_mode:
                rotation_out.append((rotations[i]))
                
        ave_translation = np.array([np.mean(x_out), np.mean(y_out)])
        return Pose(np.mean(rotation_out), ave_translation)
    ...
    def UpdateScan(self, scan_msg):
        ...
        for p in range(len(self.particles[:])):
            prob.append(self.particles[p].weight)

        prob = prob/np.sum(prob)

        part_ind = np.random.choice(NUM_PARTICLES, NUM_PARTICLES, p = prob)

        part = []
        for p in range(NUM_PARTICLES):
            part.append(copy.deepcopy(self.particles[part_ind[p]]))

        for p in range(NUM_PARTICLES):
            self.particles[p] = part[p]
