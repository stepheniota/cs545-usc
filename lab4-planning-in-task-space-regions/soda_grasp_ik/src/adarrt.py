...
class AdaRRT():
    ...
    def build(self):
        ...
        for k in range(self.max_iter):
            ...
            randomizer = np.random.choice(2, 1, p=[0.8, 0.2])
            if randomizer == 0:
                random_sample = self._get_random_sample()
            else:
                random_sample = self._get_random_sample_near_goal()

            n_neighbor = self._get_nearest_neighbor(random_sample)
            new_node = self._extend_sample(random_sample, n_neighbor)

            if new_node and self._check_for_completion(new_node):
                # FILL in your code here
                goal = new_node.add_child(self.goal.state)
                return self._trace_path_from_start(goal)
                #return path
    ...
    def _get_random_sample(self):
        ...
        """
        # FILL in your code here
        random_sample = np.empty(self.start.state.shape[0])
        for i in range(self.start.state.shape[0]):
            sample_range = np.arange(self.joint_lower_limits[i],self.joint_upper_limits[i],0.1)
            random_sample[i] = np.random.choice(sample_range)
    ...
    def _get_random_sample_near_goal(self):
        ...
        random_sample = np.empty(self.start.state.shape[0])
        for i in range(self.start.state.shape[0]):
            sample_range = np.arange(self.goal.state[i]-0.05, self.goal.state[i]+0.05, 0.1)
            random_sample[i] = np.random.choice(sample_range)
    ...
    def _extend_sample(self, sample, neighbor):
        ...
        if np.linalg.norm(sample - neighbor.state) <= self.step_size:
            ...
            if self._check_for_collision(new_state):
                return
            node = neighbor.add_child(new_state)
        else:
            new_state = neighbor.state \
                        + ((sample - neighbor.state) / np.linalg.norm(sample - neighbor.state)) \
                        * self.step_size
            if self._check_for_collision(new_state):
                return
            node = neighbor.add_child(new_state)
    ...
    def _check_for_completion(self, node):
        ...
        """
        # FILL in your code here
        return True if np.linalg.norm(self.goal.state - node.state) <= self.goal_precision else False
