...
def createBw():
    ...
    soda_can_height = 0.12
    Bw  = np.zeros((6,2))
    Bw[2,:] = [-soda_can_height / 4, soda_can_height / 4]
    Bw[-1,:] = [-np.pi / 8, 5 * np.pi / 8]  #[np.pi / 8, 5 * np.pi / 8]
    # [-np.pi, np.pi]
    # [-np.i/8, np.pi*5/8] 
    ###
    return Bw
...
def main(if_sim):
    ...
    n_iter = range(20)
    delta_z = 0.1  #0.5
    delta_x = np.array([0, 0, 0, -delta_z, 0, 0]).reshape((-1, 1))
    q = arm_skeleton.get_positions()
    
    for i in n_iter:
        jac = arm_skeleton.get_jacobian(hand.get_endeffector_body_node())
        jac_pinv = np.linalg.pinv(jac)
        delta_q = np.matmul(jac_pinv, delta_x)

        # update robot arm state to calculate new Jacobian
        q -= delta_q.reshape((-1))
        ada.set_positions(q)
